...
import { useAuth } from '@/components/Providers'; // ADDED IMPORT
...
export default function MyProjectsPage() {
  const supabaseClient = createClient(); // Keep this for direct Supabase calls if needed outside of auth context
        ...
        allProjectsData.map(async (p) => {
          ...
          if (p.owner_user_id === user.id) {
            ...
            if (memberRoleData && memberRoleData.role) {
              const dbRole = memberRoleData.role; // e.g., 'Freelancer' from DB
              // Try to match with ProjectRole enum values for consistency
              if (dbRole.toLowerCase() === ProjectRole.CLIENT.toLowerCase()) {
                userRoleForProject = ProjectRole.CLIENT;
              } else if (dbRole.toLowerCase() === ProjectRole.Freelancer.toLowerCase()) {
                userRoleForProject = ProjectRole.Freelancer;
              } else if (dbRole.toLowerCase() === ProjectRole.ProjectManager.toLowerCase()) {
                userRoleForProject = ProjectRole.ProjectManager;
              } else if (dbRole.toLowerCase() === ProjectRole.Viewer.toLowerCase()) {
                userRoleForProject = ProjectRole.Viewer;
              } else {
                userRoleForProject = dbRole; // Fallback to the exact string from DB if no enum match
              }
            } else if (p.is_public) {
