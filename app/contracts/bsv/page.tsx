...
// Convert pricing data to BSV pricing
const contracts = pricingCategories.flatMap(category =>
  category.items.map(item => {
    ...
    return {
      ...
      price: convertToCrypto(item.price, 'BSV'),
      gbpPrice: item.price,
      unit: item.unit,
...
const blockchainMeta = getBlockchainMeta('BSV');

export default function BSVContractsPage() {
      ...
      <div className="px-4 md:px-8 py-16">
        <div className="w-full">
          ...
          <div className="mb-12 border-b border-zinc-900 pb-4">
            ...
            <p className="text-xs text-yellow-500 uppercase tracking-widest mt-2 font-mono">
              {blockchainMeta.tagline}
            </p>
            <p className="text-xs text-zinc-500 uppercase tracking-widest mt-1 font-mono">
              {contracts.length} services · Priced in BSV
            </p>
          </div>
              ...
              <button
                ...
                className={`px-3 py-1 text-[10px] border transition-all font-mono uppercase tracking-tight ${
                  selectedCategory === cat
                    ...
                    : 'border-zinc-900 text-zinc-500 hover:border-yellow-500 hover:text-yellow-500 hover:bg-zinc-900/50'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative mb-8">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-900 text-white pl-12 pr-4 py-3 text-sm font-mono uppercase tracking-wide focus:border-yellow-500 focus:outline-none transition-colors"
            />
          </div>
          ...
          <div className="mb-6 text-xs text-zinc-600 font-mono uppercase flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <span className="text-yellow-500">BSV</span>
              <span className="text-zinc-800">|</span>
                ...
                <div className="divide-y divide-zinc-900">
                  {group.items.map((item, j) => (
                    ...
                    >
                      ...
                      <div className="col-span-2 text-sm font-bold text-right font-mono text-yellow-500 group-hover:text-yellow-400">
                        {item.price}
                      </div>
                      ...
                      <div className="col-span-3 text-right">
                        <Link
                          ...
                          className="inline-block px-3 py-1 text-[10px] border border-zinc-800 text-zinc-500 hover:border-yellow-500 hover:text-yellow-500 transition-all font-mono uppercase"
                        >
                          View Contract →
                        </Link>
                      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div>
                <p className="text-xs text-zinc-500 mb-2 font-mono uppercase">
                  All contracts on BSV blockchain. Instant settlement.
                </p>
                <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-tight">
                  Sub-penny fees · Unlimited scale · Verifiable delivery
                </p>
              </div>
              <div className="flex gap-4">
                <Link
                  href="/contracts"
                  className="px-6 py-3 border border-zinc-900 text-zinc-400 text-[10px] font-bold uppercase tracking-widest hover:border-yellow-500 hover:text-yellow-500 transition-all font-mono"
                >
                  View All Chains
                </Link>
                <a
