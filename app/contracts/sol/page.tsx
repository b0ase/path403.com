...
// Convert pricing data to SOL pricing
const contracts = pricingCategories.flatMap(category =>
  category.items.map(item => {
    ...
    return {
      ...
      price: convertToCrypto(item.price, 'SOL'),
      gbpPrice: item.price,
      unit: item.unit,
...
const blockchainMeta = getBlockchainMeta('SOL');

export default function SOLContractsPage() {
      ...
      <div className="px-4 md:px-8 py-16">
        <div className="w-full">
          ...
          <div className="mb-12 border-b border-zinc-900 pb-4">
            ...
            <p className="text-xs text-purple-500 uppercase tracking-widest mt-2 font-mono">
              {blockchainMeta.tagline}
            </p>
            <p className="text-xs text-zinc-500 uppercase tracking-widest mt-1 font-mono">
              {contracts.length} services · Priced in SOL
            </p>
          </div>
              ...
              <button
                ...
                className={`px-3 py-1 text-[10px] border transition-all font-mono uppercase tracking-tight ${
                  selectedCategory === cat
                    ...
                    : 'border-zinc-900 text-zinc-500 hover:border-purple-500 hover:text-purple-500 hover:bg-zinc-900/50'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative mb-8">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-900 text-white pl-12 pr-4 py-3 text-sm font-mono uppercase tracking-wide focus:border-purple-500 focus:outline-none transition-colors"
            />
          </div>
                ...
                <div className="divide-y divide-zinc-900">
                  {group.items.map((item, j) => (
                    ...
                    >
                      ...
                      <div className="col-span-2 text-sm font-bold text-right font-mono text-purple-500 group-hover:text-purple-400">
                        {item.price}
                      </div>
                      ...
                      <div className="col-span-3 text-right">
                        <Link
                          ...
                          className="inline-block px-3 py-1 text-[10px] border border-zinc-800 text-zinc-500 hover:border-purple-500 hover:text-purple-500 transition-all font-mono uppercase"
                        >
                          View Contract →
                        </Link>
                      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div>
                <p className="text-xs text-zinc-500 mb-2 font-mono uppercase">
                  All contracts on Solana. Lightning fast.
                </p>
                <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-tight">
                  High throughput · Low latency · Verifiable delivery
                </p>
              </div>
              <div className="flex gap-4">
                <Link
                  href="/contracts"
                  className="px-6 py-3 border border-zinc-900 text-zinc-400 text-[10px] font-bold uppercase tracking-widest hover:border-purple-500 hover:text-purple-500 transition-all font-mono"
                >
                  View All Chains
                </Link>
                <a
                  href="mailto:richard@b0ase.com"
                  className="px-6 py-3 bg-purple-500 text-white text-[10px] font-bold uppercase tracking-widest hover:bg-purple-400 transition-all font-mono"
                >
                  Contact Us
                </a>
              </div>
