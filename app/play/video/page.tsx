...
export default function MusicVideoPage() {
  ...
  const [mouseHover, setMouseHover] = useState(true); // Start with 3D control on
  ...
  // Music-reactive states
  const { audioData, isAnalyzing, isPlaying } = usePersistentMusic();
  const [beatScale, setBeatScale] = useState(1);
  const [textRotation, setTextRotation] = useState(0);
  const [textScale, setTextScale] = useState(0);
  const [modelRotation, setModelRotation] = useState(0);
  const [modelFloatY, setModelFloatY] = useState(0);
  const [modelJudderX, setModelJudderX] = useState(0);
  const [modelJudderZ, setModelJudderZ] = useState(0);
  const lastBeatTime = useRef(0);
  const beatThreshold = useRef(0.6);

  // Ensure client-side only rendering to prevent hydration issues
  useEffect(() => {
    setIsClient(true);

    // Start with both models for more dramatic effect
    setModelType('both');
    // Enable 3D control and zoom for dynamic camera
    setMouseHover(true);
    setZoomLevel(80);
  }, []);
  ...
  // Chaotic random animations - not synced to music
  useEffect(() => {
    if (!isClient) return;

    const chaosInterval = setInterval(() => {
      // Random text clipping with varying intensity
      if (Math.random() > 0.8) {
        const clipAmount = Math.random() * 30; // 0 to 30% clip
        setTextScale(clipAmount);
        setTimeout(() => setTextScale(0), 300 + Math.random() * 700);
      }

      // Random text skew/cut - less frequent
      if (Math.random() > 0.75) {
        const skews = [0, 45, -45, 90, -90];
        setTextRotation(skews[Math.floor(Math.random() * skews.length)]);
      }

      // Random model changes
      if (Math.random() > 0.9) {
        setModelType(prev => {
          const types = ['wireframe', 'gundam', 'both'];
          const current = types.indexOf(prev);
          const next = (current + 1 + Math.floor(Math.random() * 2)) % types.length;
          return types[next] as any;
        });
      }

      // Random visual toggles
      if (Math.random() > 0.85) {
      ...
      // Chaotic zoom changes - much less frequent and smaller range
      if (Math.random() > 0.95) {
        const targetZoom = 70 + Math.random() * 40; // 70-110
        setZoomLevel(targetZoom);
      }

      // Wild judder effects - less frequent
      if (Math.random() > 0.75) {
        setModelJudderX((Math.random() - 0.5) * 15);
        setModelJudderZ((Math.random() - 0.5) * 15);
        setTimeout(() => {
          setModelJudderX(0);
          setModelJudderZ(0);
        }, 100 + Math.random() * 400);
      ...
      // Erratic model rotation - slower
      setModelRotation(prev => prev + (Math.random() - 0.5) * 15);

      // Random floating - smoother
      const floatAmount = (Math.random() - 0.5) * 10;
      setModelFloatY(floatAmount);
    ...
    }, 1000 + Math.random() * 2000); // Random interval between 1000-3000ms

    return () => clearInterval(chaosInterval);
  }, [isClient]);

  // Auto-cycle through different states (disabled when music playing)
  useEffect(() => {
    if (!autoCycle || !isClient || isPlaying) return;
  ...
  }, [autoCycle, fontOptions.length, isClient, isPlaying]);
  ...
  return (
    ...
    >
          ...
          {isClient && (
            <div className="w-full h-full">
              <WireframeAnimation
                ...
                modelFloatY={modelFloatY}
                modelJudderX={modelJudderX}
                modelJudderZ={modelJudderZ}
              />
            </div>
          )}
      ...
      <div className={`relative ${mouseHover ? 'z-[55]' : 'z-10'} overflow-x-hidden ${mouseHover ? 'pointer-events-none' : 'pointer-events-auto'}`} style={{ paddingTop: '80px' }}>
        ...
        >
          ...
          <div className="overflow-x-hidden">
            <div>
              ...
              {/* Main Title - Music Reactive */}
              <div className="mb-6" style={{
                clipPath: `inset(0 ${textScale}% 0 ${textScale}%)`,
              ...
              {/* Subtitles with better spacing - Music Reactive */}
              <div className="space-y-3 w-full max-w-6xl" style={{
                clipPath: `inset(${textScale / 2}% 0 ${textScale / 2}% 0)`,
